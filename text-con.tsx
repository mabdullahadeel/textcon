...
export function createContextStore<Store>(initialState: Store, options: ContextOptions = {}) {
  let globalStore: Store | undefined = options.global ? initialState : undefined;
  ...
  function useStoreData({
    ...
    const subscribers = useRef(new Set<(state: Store) => void>(defaultSubscriber));
  ...
  function Provider({ children }: { children: React.ReactNode }) {
    return (
      <StoreContext.Provider value={useStoreData({ defaultSubscriber: [observable.broadcast] })}>
        {children}
  ...
  function subscribeExternal<SelectorOutput = Store>(
    selector: (store: Store) => SelectorOutput = store => store as unknown as SelectorOutput,
    ...
    const shouldSendUpdates = (newSelectedState: SelectorOutput) => {
      if (
        ...
        (options.compare && !options.compare(lastSelectedState, newSelectedState)) ||
